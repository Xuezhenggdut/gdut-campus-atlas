...
const internalNames=new Set(['创新大道','知行大道','知行大道（南1门段）','体育馆—网球场连接路','求是路','研学路','科研楼组团横向道路','教学楼组团横向道路','西区滨水路','环教路']);
export function mobilityRoutes(){return {
 external:roads.filter(r=>r.name==='大学城外环西路').map(r=>mobilityRoute(r.name!,r.width,r.points.map(toWorld))),
