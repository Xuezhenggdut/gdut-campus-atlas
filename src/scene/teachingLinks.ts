...
export function makeTeachingLinks(p:Parts){
 ...
 for(const {x,z0,z1} of teachingLinkSpans()){
  for(const y of levels){
   const deckWidth=y===levels[0]?teachingPlatformWidth:width;
  ...
  for(const z of [z0+.4,z1-.4])for(const side of [-1,1])p.cylinder(.32,levels[0]-.5,x+side*(teachingPlatformWidth/2-.4),(levels[0]+.5)/2,z,'#ded7bd',.32,10);
 }
}
